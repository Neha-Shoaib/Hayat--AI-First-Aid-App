...
export default function HayatMasterApp() {
  ...
  const fetchLocation = () => {
    ...
    if ("geolocation" in navigator) {
      ...
      setLocation({ lat: "Not Supported", lng: "" });
      setLocLoading(false);
    }
  };

  // DUAL ENGINE AUDIO FIX (Google MP3 + Native Fallback)
  ...
  const ui = lang === "Urdu" ? {
    callBtn: "1122 پر کال کریں", locating: "لوکیشن تلاش...", cprStart: "سی پی آر شروع کریں", cprStop: "سی پی آر روکیں",
    quickScenarios: "فوری صورتحال:",
    scenarios: [ { title: "🩸 گہرا کٹ / خون", q: "گہرا کٹ لگا ہے اور خون بہہ رہا ہے" }, { title: "🔥 جل جانا", q: "آگ سے جل گیا ہے" }, { title: "🫁 سانس رکنا", q: "سانس نہیں آ رہی" }, { title: "⚡ کرنٹ", q: "بجلی کا کرنٹ لگا ہے" } ],
    ...
    callBtn: "CALL 1122", locating: "Locating...", cprStart: "START CPR", cprStop: "STOP CPR", quickScenarios: "Quick:",
    scenarios: [ { title: "🩸 Severe Cut", q: "Severe bleeding" }, { title: "🔥 Burn", q: "Burn from fire" }, { title: "🫁 Choking", q: "Choking" }, { title: "⚡ Shock", q: "Electric shock" } ],
  ...
  return (
    <main className={`min-h-screen w-full transition-colors duration-200 overflow-x-hidden ${isDark ? "bg-slate-950 text-slate-100" : "bg-slate-50 text-slate-900"} ${lang === "Urdu" ? "font-urdu" : "font-inter"}`}>
      <div className="w-full max-w-2xl mx-auto px-3 sm:px-6 py-4 flex flex-col gap-4">
        ...
        <button onClick={toggleCPR} className={`w-full py-3.5 px-4 rounded-xl flex items-center justify-center gap-2 font-black text-sm sm:text-base border font-inter ${cprActive ? "bg-red-600 text-white border-red-600 shadow-lg" : (isDark ? "bg-slate-900 border-slate-800" : "bg-white border-slate-300")}`}>
        ...
        <section>
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-2.5">
            {ui.scenarios.map((item, idx) => (
              <button key={idx} onClick={() => submitEmergency(null, item.q)} className={`w-full p-3 rounded-xl border text-xs font-bold transition hover:border-red-500 ${lang === "Urdu" ? "text-right" : "text-left"} ${isDark ? "bg-slate-900/80 border-slate-800" : "bg-white border-slate-300"}`}>
                {item.title}
              </button>
            ))}
          </div>
        ...
        <section className={`p-5 sm:p-8 rounded-2xl border text-center flex flex-col items-center gap-4 ${isDark ? "bg-slate-900/40 border-slate-800" : "bg-white border-slate-200"}`}>
          {isRecording ? (
            <div className="flex flex-col items-center gap-3 w-full">
              ...
              <button onClick={stopRecording} className="w-full sm:w-auto bg-red-600 text-white px-8 py-3.5 rounded-full font-bold text-sm flex items-center justify-center gap-2"><Square className="h-5 w-5" /> {ui.stopRecording}</button>
            </div>
          ) : (
            <div className="flex flex-col items-center gap-3 w-full">
              <button onClick={startRecording} disabled={loading} className="w-20 h-20 sm:w-24 sm:h-24 rounded-full bg-red-600 text-white flex items-center justify-center shadow-xl hover:scale-105 transition-all"><Mic className="h-10 w-10" /></button>
              <h2 className="font-black text-lg">{ui.micMain}</h2>
              <div className="w-full flex flex-col sm:flex-row gap-2 pt-4">
                <input type="text" placeholder={ui.placeholder} value={inputText} onChange={(e) => setInputText(e.target.value)} onKeyDown={(e) => e.key === "Enter" && submitEmergency(null, inputText)} className={`w-full flex-1 rounded-xl px-4 py-3.5 text-sm border focus:outline-none ${lang === "Urdu" ? "text-right" : "text-left"} ${isDark ? "bg-slate-950 border-slate-800 text-white" : "bg-slate-100 border-slate-300"}`} />
        ...
        {loading && <div className="text-center text-red-500 font-bold flex justify-center items-center gap-2 animate-pulse text-sm py-2"><Sparkles className="h-5 w-5 animate-spin" /> {ui.processing}</div>}
        ...
        {result && (
          <section className={`rounded-2xl border p-4 sm:p-6 flex flex-col gap-5 shadow-lg ${isDark ? "bg-slate-900 border-slate-800" : "bg-white border-slate-200"}`}>
            <div className={`flex flex-col sm:flex-row gap-3 sm:items-center sm:justify-between border-b border-slate-700/20 pb-4 ${lang === "Urdu" ? "sm:flex-row-reverse" : ""}`}>
              <h3 className={`font-bold text-base ${lang === "Urdu" ? "text-right" : "text-left"}`}>{result.title}</h3>
              <button onClick={() => playAudio(result.spokenSummary)} className={`w-full sm:w-auto flex justify-center items-center gap-2 bg-red-600 text-white px-4 py-2.5 rounded-lg text-sm font-black shadow ${lang === "Urdu" ? "flex-row-reverse" : ""}`}>
                {isPlayingAudio ? <VolumeX className="h-4 w-4" /> : <Volume2 className="h-4 w-4" />} {isPlayingAudio ? ui.stopAudio : ui.listen}
              </button>
            </div>

            <div className="flex flex-col gap-3">
              <p className={`text-xs font-black uppercase text-emerald-500 flex items-center gap-1.5 ${lang === "Urdu" ? "flex-row-reverse justify-start" : ""}`}><CheckCircle2 className="h-5 w-5" /> {ui.actionSteps}</p>
              {result.dos?.map((step, idx) => (
                <div key={idx} className={`p-3.5 rounded-xl border flex gap-3 ${lang === "Urdu" ? "flex-row-reverse text-right" : ""} ${isDark ? "bg-slate-950/60 border-emerald-950/40" : "bg-emerald-50/60 border-emerald-200"}`}>
                  <span className="h-6 w-6 rounded-full bg-emerald-500/20 text-emerald-500 flex items-center justify-center text-sm font-black mt-0.5">{idx + 1}</span>
            ...
            <div className="flex flex-col gap-3">
              <p className={`text-xs font-black uppercase text-rose-500 flex items-center gap-1.5 ${lang === "Urdu" ? "flex-row-reverse justify-start" : ""}`}><AlertOctagon className="h-5 w-5" /> {ui.donts}</p>
            ...
            {result.videoId && (
              <a href={`https://www.youtube.com/watch?v=${result.videoId}`} target="_blank" rel="noopener noreferrer" className={`mt-2 w-full bg-[#FF0000] hover:bg-red-700 text-white p-3.5 rounded-xl flex items-center justify-center gap-2 font-bold shadow-md ${lang === "Urdu" ? "flex-row-reverse font-inter" : ""}`}>
                <Youtube className="h-6 w-6" /> <span className="truncate">{ui.watchVideo}</span> <ExternalLink className="h-4 w-4 opacity-70" />
              </a>
            )}
          </section>
        )}
